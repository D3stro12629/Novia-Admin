<template>
  <DashboardLayout>
    <div class="premium-shell" :class="{ 'is-loading': subjectStore.isLoading }">
            ...
            <tbody>
              <template v-if="subjectStore.isLoading">
                <tr v-for="i in 6" :key="'table-s-' + i">
                  ...
                  <td><BaseSkeleton width="110px" height="16px" radius="4px" /></td>
                  <td class="text-end">
              ...
              <template v-else>
                <tr v-for="subject in paginatedSubjects" :key="subject.id" class="hover-row">
                  <td><span class="id-tag">#{{ subject.id }}</span></td>
                  <td>
                    <div class="member-cell">
                      <div class="category-img-wrapper">
                        <div class="img-placeholder">{{ subject.name.charAt(0) }}</div>
                      </div>
                  ...
                  <td class="text-end">
                    <div class="action-btns">
                      <button class="action-icon" @click="handleView(subject)" title="View Details"><i class="bi bi-eye"></i></button>
                      <button class="action-icon" @click="handleEdit(subject)" title="Edit"><i class="bi bi-pencil"></i></button>
                      <button class="action-icon delete" @click="handleDelete(subject)" title="Delete"><i class="bi bi-trash"></i></button>
                    </div>
                  </td>
                </tr>
              </template>
              ...
              <div class="modal-content compact-padding">
                ...
                <div class="form-body">
                  <div class="input-group-custom">
                    ...
                    <input v-model="form.name" placeholder="e.g. Mathematics / Physics" :class="{ 'has-error': errors.name }" @blur="validate('name')" />
            ...
            <div class="modal-surface compact-modal">
              ...
              <div class="modal-content compact-padding">
                ...
                <div class="modal-footer-refined">
                  ...
                  <button class="btn-danger" :disabled="subjectStore.isProcessing" @click="confirmDelete">Yes, Delete</button>
                </div>
              </div>
            </div>
          </div>
...
const { errors, validateField: validate, validate: validateAll, reset: resetValidation } = useFormValidation(form, {
    name: [validationRules.required('Subject name is required'), validationRules.maxLength(255)]
})
...
const filteredSubjects = computed(() => {
    if (!searchQuery.value) return subjectStore.subjects || []
    return (subjectStore.subjects || []).filter(s => s.name.toLowerCase().includes(searchQuery.value.toLowerCase()))
})
...
const saveSubject = async () => {
    ...
    try {
        isEditing.value ? await subjectStore.editSubject(selectedItem.value.id, { name: form.name }) : await subjectStore.createSubject({ name: form.name })
        showFormModal.value = false; await subjectStore.fetchSubjects({ force: true }); toast.success('Saved Successfully')
...
const confirmDelete = async () => {
    try {
        ...
        showDeleteModal.value = false; await subjectStore.fetchSubjects({ force: true }); toast.success('Deleted Successfully')
...
.premium-shell {
  ...
  background: var(--bg); color: var(--text-main); font-family: 'Plus Jakarta Sans', sans-serif; min-height: 100vh; padding: 2rem;
}

/* Header */
.admin-header { margin-bottom: 2rem; }
.header-content { display: flex; justify-content: space-between; align-items: flex-end; }
.title-stack h1 { font-size: 2.5rem; font-weight: 800; letter-spacing: -1.5px; display: flex; align-items: center; gap: 1rem; margin: 0; }
...
.search-wrapper { position: relative; background: var(--surface); border: 1px solid var(--border); border-radius: 14px; width: 300px; height: 48px; }
.search-wrapper input { width: 100%; background: transparent; border: none; color: white; padding: 0 1rem 0 2.8rem; outline: none; height: 100%; font-size: 0.9rem; }
.search-icon { position: absolute; left: 1rem; top: 50%; transform: translateY(-50%); color: var(--text-muted); }
.search-shortcut { position: absolute; right: 0.8rem; top: 50%; transform: translateY(-50%); background: #27272a; color: #71717a; padding: 2px 6px; border-radius: 4px; font-size: 0.7rem; border: 1px solid #3f3f3f; font-family: 'JetBrains Mono', monospace; }

.primary-add-btn { background: var(--accent); color: white; border: none; height: 48px; padding: 0 1.5rem; border-radius: 14px; font-weight: 700; display: flex; align-items: center; gap: 8px; transition: 0.2s; }
...
.stat-icon { width: 56px; height: 56px; border-radius: 18px; display: flex; align-items: center; justify-content: center; font-size: 1.4rem; flex-shrink: 0; }
.stat-icon.blue { background: rgba(99, 102, 241, 0.1); color: var(--accent); }
.stat-icon.green { background: rgba(34, 197, 94, 0.1); color: #22c55e; }
.stat-icon.purple { background: rgba(168, 85, 247, 0.1); color: #a855f7; }
.stat-data .value { font-size: 1.75rem; font-weight: 800; display: block; }
...
.premium-table th { text-align: left; padding: 1rem 1.5rem; font-size: 0.7rem; text-transform: uppercase; color: var(--text-muted); border-bottom: 1px solid var(--border); letter-spacing: 1.5px; font-weight: 700; }
...
.img-placeholder { width: 100%; height: 100%; display: flex; align-items: center; justify-content: center; background: var(--accent); color: white; font-weight: 800; font-size: 1.2rem; }
...
.btn-primary, .btn-secondary, .btn-danger { padding: 0.8rem; border-radius: 12px; font-weight: 700; cursor: pointer; border: none; transition: 0.2s; }
...
.large-text { color: var(--accent); font-size: 1.1rem !important; font-weight: 700; }
.modal-footer-refined { display: flex; gap: 1rem; margin-top: 2rem; }
.full-width { width: 100%; }
.delete-warning { text-align: center; background: rgba(239, 68, 68, 0.05); padding: 1rem; border-radius: 14px; color: #fca5a5; margin-bottom: 1rem; }

/* Pagination */
.premium-pagination-footer { display: flex; justify-content: space-between; align-items: center; padding: 1.25rem 1.5rem; background: rgba(0, 0, 0, 0.2); border-top: 1px solid var(--border); }
