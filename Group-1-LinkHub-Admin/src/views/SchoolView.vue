...
onMounted(() => schoolStore.fetchSchools())
...
const filteredSchools = computed(() => {
    ...
    return (schoolStore.schools || []).filter(c => c.name.toLowerCase().includes(searchQuery.value.toLowerCase()))
})
...
const saveSchool = async () => {
    ...
    try {
        ...
        showFormModal.value = false; await schoolStore.fetchSchools({ force: true }); toast.success('Saved Successfully')
...
const confirmDelete = async () => {
    try {
        ...
        showDeleteModal.value = false; await schoolStore.fetchSchools({ force: true }); toast.success('Deleted Successfully')
...
.title-stack h1 { font-size: 2.5rem; font-weight: 800; letter-spacing: -1.5px; display: flex; align-items: center; gap: 1rem; margin: 0; line-height: 1.1; }
...
.primary-add-btn { background: var(--accent); color: white; border: none; height: 48px; padding: 0 1.5rem; border-radius: 14px; font-weight: 700; display: flex; align-items: center; gap: 10px; cursor: pointer; transition: 0.2s; }
...
.premium-table th, .premium-table td { padding: 1rem 1.5rem; border-bottom: 1px solid var(--border); }
.premium-table th { background: rgba(255, 255, 255, 0.01); text-align: left; font-size: 0.7rem; text-transform: uppercase; color: var(--text-muted); font-weight: 700; letter-spacing: 1.5px; }
...
.member-cell { display: flex; align-items: center; gap: 1rem; min-width: 0; }
...
.name-text { font-weight: 700; font-size: 1rem; color: #fff; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }

.status-pill { padding: 5px 12px; border-radius: 8px; font-size: 0.65rem; font-weight: 800; border: 1px solid currentColor; text-transform: uppercase; }
.status-active { background: rgba(34, 197, 94, 0.1); color: #34d399; }
.date-text { color: #f1f5f9; font-weight: 600; font-size: 0.95rem; }
...
.modal-surface.compact-modal { max-width: 400px; border-radius: 28px; background: #0c0c0e; border: 1px solid var(--border); width: 90%; position: relative; overflow: hidden; }
...
.large-text { font-size: 1.1rem !important; color: var(--accent); font-weight: 700; }
