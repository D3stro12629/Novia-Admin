    ...
    <DashboardLayout>
        <div class="premium-shell" :class="{ 'is-loading': profileStore.isLoading }">
            <div class="content-limiter">
                ...
                <header class="profile-header">
                    ...
                    <div class="profile-meta-bar">
                        ...
                        <div class="user-details">
                            ...
                            <div class="meta-info">
                                <span><i class="bi bi-fingerprint"></i> ID: {{ profileStore.user?.id || '---' }}</span>
                        ...
                        <div class="header-actions">
                            <button class="edit-core-btn" @click="openEditModal">
                ...
                <div class="stats-grid">
                    ...
                    <div class="stat-item">
                        ...
                        <div class="s-labels"><span class="s-top">Status</span><span class="s-val">Active</span></div>
                    </div>
                    <div class="stat-item">
            ...
            <Teleport to="body">
                        ...
                        <div class="env-modal-surface mini-modal">
                            ...
                            <div class="env-modal-footer">
                                <div class="env-footer-row">
                                    ...
                                    <button class="env-btn-danger-solid" @click="confirmDeleteCover" :disabled="profileStore.isProcessing">
                ...
                <Transition name="ref-modal">
                    <div v-if="showAvatarDeleteModal" class="modal-backdrop" @click.self="showAvatarDeleteModal = false">
                        <div class="env-modal-surface mini-modal">
                            <div class="env-modal-header">
                                <div class="env-header-left">
                                    <div class="env-icon-box danger"><i class="bi bi-exclamation-triangle"></i></div>
                            ...
                            <div class="env-modal-footer">
                                <div class="env-footer-row">
                                    ...
                                    <button class="env-btn-danger-solid" @click="confirmDeleteAvatar" :disabled="profileStore.isProcessing">
                            ...
                            <div class="env-modal-header">
                                <div class="env-header-left">
                                    ...
                                    <div class="env-header-text">
                                        ...
                                        <p>Modify your environment background</p>
                                    </div>
                                </div>
                            ...
                            <div class="core-modal-body">
                                <div class="core-grid">
                                    ...
                                    <div class="core-input"><label>Portfolio</label><input
                                            v-model="personalForm.portfolio_link" /></div>
                                </div>
                            </div>
...
const { errors: passwordFormErrors, validate: validateAllPassword, validateField: validatePasswordField, reset: resetPasswordValidation } = useFormValidation(passwordForm, {
    ...
    new_pass: [
        ...
        validationRules.pattern('^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d)(?=.*[^A-Za-z0-9]).+$', 'Include Upper, Lower, Number, Symbol'),
...
const confirmDeleteAvatar = async () => {
    try {
        ...
        toast.success('Identity avatar removed')
...
const handleFileUpload = async (e) => {
    ...
    if (file) {
        try {
            ...
            toast.success('Identity avatar updated')
...
const lastLoginDisplay = computed(() => profileStore.user?.last_login_at ? new Date(profileStore.user.last_login_at).toLocaleDateString() : 'Active Now')
const profileCompletion = computed(() => {
    const fields = ['full_name', 'email', 'phone', 'avatar', 'cover']
    return Math.round((fields.filter(f => profileStore.user?.[f]).length / fields.length) * 100)
})

watch(activeTab, (t) => { if (t === 'security') { resetPasswordValidation(); showPasswords.old = showPasswords.new = showPasswords.confirm = false } })
watch([showEditModal, showCoverModal, showAvatarDeleteModal, showCoverDeleteModal], (v) => document.body.style.overflow = v.some(x => x) ? 'hidden' : '')
</script>
...
.s-icon.purple { background: rgba(168, 85, 247, 0.1); color: #a855f7; }
.s-icon.blue { background: rgba(59, 130, 246, 0.1); color: #3b82f6; }
.s-icon.green { background: rgba(34, 197, 94, 0.1); color: #22c55e; }

.s-labels .s-top { display: block; font-size: 0.7rem; color: var(--text-dim); font-weight: 600; }
.s-labels .s-val { font-size: 1.2rem; font-weight: 700; }
...
.nav-link.active { background: var(--surface); color: var(--accent-color); }
...
.data-card { background: var(--bg-dark); border: 1px solid var(--border-color); padding: 1rem; border-radius: 12px; }
.data-card.span-2 { grid-column: span 2; }
.data-card label { display: block; font-size: 0.6rem; text-transform: uppercase; color: var(--text-dim); margin-bottom: 5px; font-weight: 700; }
.data-card p { margin: 0; font-weight: 600; font-size: 0.95rem; }
.highlight { color: var(--accent-color); font-size: 1.1rem !important; }
...
.env-modal-surface { width: 480px; background: #18181b; border: 1px solid #27272a; border-radius: 20px; color: white; overflow: hidden; }
.mini-modal { width: 380px !important; }

.env-modal-header { display: flex; justify-content: space-between; align-items: center; padding: 1.25rem; }
.env-header-left { display: flex; align-items: center; gap: 15px; }
.env-icon-box { width: 40px; height: 40px; background: #27272a; border: 1px solid #3f3f46; border-radius: 10px; display: flex; align-items: center; justify-content: center; font-size: 1.2rem; }
.env-icon-box.danger { color: #ef4444; border-color: rgba(239, 68, 68, 0.2); background: rgba(239, 68, 68, 0.1); }
.env-header-text h3 { margin: 0; font-size: 1.1rem; font-weight: 700; }
.env-header-text p { margin: 0; font-size: 0.85rem; color: #a1a1aa; }

.env-close-btn { background: transparent; border: none; color: #71717a; cursor: pointer; font-size: 1.2rem; }

.env-modal-body { padding: 0 1.25rem 1.25rem; text-align: center; }
.env-preview-zone { width: 100%; height: 180px; background: #09090b; border-radius: 12px; overflow: hidden; margin-bottom: 1rem; cursor: pointer; border: 1px solid #27272a; display: flex; align-items: center; justify-content: center; }
.env-img-preview { width: 100%; height: 100%; object-fit: contain; }
.env-empty-preview { color: #3f3f46; font-size: 2rem; }
.env-choose-btn { width: 100%; background: #09090b; border: 1px solid #27272a; color: white; padding: 12px; border-radius: 10px; font-weight: 600; cursor: pointer; }
.env-hint-text { font-size: 0.75rem; color: #71717a; margin-top: 12px; }

.env-modal-footer { padding: 1.25rem; background: rgba(0, 0, 0, 0.2); display: flex; flex-direction: column; gap: 10px; }
.env-footer-row { display: grid; grid-template-columns: 1fr 1fr; gap: 8px; }

.env-btn-dark { background: #18181b; border: 1px solid #27272a; color: white; padding: 8px; border-radius: 8px; font-size: 0.85rem; font-weight: 600; cursor: pointer; }
.env-btn-danger-solid { background: #ef4444; color: white; border: none; padding: 8px; border-radius: 8px; font-size: 0.85rem; font-weight: 600; cursor: pointer; }
.env-btn-danger-solid:disabled { opacity: 0.4; cursor: not-allowed; }

.env-btn-submit { width: 100%; background: #d4d4d8; color: #18181b; border: none; padding: 12px; border-radius: 10px; font-weight: 700; cursor: pointer; }

/* EDIT PROFILE MODAL */
.core-modal-surface { width: 550px; background: #0c0c0e; border: 1px solid #27272a; border-radius: 20px; overflow: hidden; }
.core-modal-header { display: flex; justify-content: space-between; align-items: center; padding: 1rem 1.25rem; border-bottom: 1px solid #27272a; }
.core-modal-header h3 { margin: 0; font-size: 1rem; color: #818cf8; display: flex; align-items: center; gap: 10px; }
.core-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 0.75rem; padding: 1.25rem; }
