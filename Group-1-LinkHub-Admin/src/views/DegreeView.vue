                  ...
                  <td>
                    <div class="member-cell">
                      <div class="category-img-wrapper">
                        <!-- We use a stylized initial placeholder to match Category visual style -->
...
const activeCount = computed(() => (degreeStore.degrees || []).filter(s => !s.status || s.status === 'Active').length)
const newlyAddedCount = computed(() => {
    const sevenDaysAgo = new Date(); sevenDaysAgo.setDate(sevenDaysAgo.getDate() - 7)
    return (degreeStore.degrees || []).filter(s => new Date(s.created_at || Date.now()) > sevenDaysAgo).length
})

const filteredDegrees = computed(() => {
    if (!searchQuery.value) return degreeStore.degrees || []
    const q = searchQuery.value.toLowerCase()
    return (degreeStore.degrees || []).filter(d => d.name.toLowerCase().includes(q))
})
