...
const newlyAddedCount = computed(() => {
    ...
    return (skillStore.skills || []).filter(s => new Date(s.created_at) > weekAgo).length
})
const filteredSkills = computed(() => {
    if (!searchQuery.value) return skillStore.skills || []
    return (skillStore.skills || []).filter(s => s.name.toLowerCase().includes(searchQuery.value.toLowerCase()))
})
